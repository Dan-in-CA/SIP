...
import web, json, re, time, datetime 
import subprocess
import gv # Get access to ospy's settings
from helpers import CPU_temperature, getIP, uptime, reboot, poweroff, timestr, jsave
...
urls.extend(['/smsa', 'plugins.sms_adj.settings', '/smsj', 'plugins.sms_adj.settings_json', '/usmsa', 'plugins.sms_adj.update'])

# Add this plugin to the home page plugins menu
gv.plugin_menu.append(['SMS Adjust Settings', '/smsa'])
...
try:
   ...
   self.add_status('SMS plugin encountered error: ' + str(err))
   sys.exit()
           ...
           if  (m['Number'] == tel1)or(m['Number'] == tel2): # If telephone is admin 1 or admin 2
             ...
             if m['State'] == "UnRead":          # If SMS is unread
                 if m['Text'] ==comm1:           # If command = comm1 (info - send SMS to admin phone1 and phone2) 
                      ...
                      if pgr != '0':
                         logline = ' {program: '+pgr+',station: '+str(gv.lrun[0])+',duration: '+timestr(gv.lrun[2])+',start: '+time.strftime("%H:%M:%S - %Y-%m-%d", start)+'}'
                      else:
                         logline = ' Last program none'
                      revision = ' Rev: '+ gv.ver_date
                      datastr = ('On '+time.strftime("%d.%m.%Y at %H:%M:%S", time.localtime(time.time()))+'. Run time: '+uptime()+' IP: '+getIP()+ logline + revision)
                      message = {
                        'Text': datastr,
                        'SMSC': {'Location': 1},
                        'Number': m['Number'],
                      ...
                      sm.SendSMS(message)
                      self.add_status('Command: '+comm1+' was processed and confirmation was sent as SMS to: '+m['Number'])
                      ...
                      sm.SendSMS(message)
                      self.add_status('Command: '+comm2+' was processed and confirmation was sent as SMS to: '+m['Number'])
                      ...
                      sm.SendSMS(message)
                      self.add_status('Command: '+comm3+' was processed and confirmation was sent as SMS to: '+m['Number'])
                      sm.DeleteSMS(m['Folder'], m['Location'])
                      self.add_status('Received SMS was deleted')
                      ...
                      sm.SendSMS(message)
                      self.add_status('Command: '+comm4+' was processed and confirmation was sent as SMS to: '+m['Number'])
                      sm.DeleteSMS(m['Folder'], m['Location'])
                      ...
                      sm.SendSMS(message)
                      self.add_status('Command: '+comm5+' was processed and confirmation was sent as SMS to: '+m['Number'])
                      sm.DeleteSMS(m['Folder'], m['Location'])
