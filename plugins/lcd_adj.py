...
from helpers import uptime, getIP, CPU_temperature
import errno

# Add a new url to open the data entry page.
urls.extend(['/lcd', 'plugins.lcd_adj.settings', '/lcdj', 'plugins.lcd_adj.settings_json', '/lcda', 'plugins.lcd_adj.update'])

# Add this plugin to the home page plugins menu
gv.plugin_menu.append(['LCD Adjust Settings', '/lcd'])
        ...
        while True:
            try:
                ...
                if datalcd['use_lcd'] != 'off':                      # if LCD plugin is enabled
                    if text_shift > 7: # Print 1-7 messages to LCD
                        text_shift = 0 
                        self.status = '' 
                ...
                self.status = '' 
                self.add_status('LCD plugin encountered error: ' + str(err))
                self._sleep(5)  
...
def get_LCD_print(self, report):  
        ...
        if datalcd['adress'] == '0x20': # range adress from PCF8574 or PCF 8574A
            ...
            self.status = ''
            self.add_status('Error: adress is not range 0x20-0x27 or 0x38-0x3F!') 
            self._sleep(5) 
            return
        
        lcd = pylcd2.lcd(adr,1) # Adress for PCF8574 = example 0x20, Bus Raspi = 1 (0 = 256MB,1=512MB)
        ...
        if report == 0:
            ...
            lcd.lcd_puts( temp+" C",2)
            self.add_status('CPU temperature: / '+temp+ ' C')
        elif report == 5:
            lcd.lcd_clear()
