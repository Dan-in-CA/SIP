...
from helpers import plugin_adjustment, prog_match, schedule_stations, log_run, stop_onrain, check_rain, jsave, data
        ...
        if gv.sd['en'] and not gv.sd['mm'] and (not gv.sd['bsy'] or not gv.sd['seq']):
            ...
            if (lt[3] * 60) + lt[4] != last_min: # only check programs once a minute
                ...
                for i, p in enumerate(gv.pd): # get both index and prog item
                    if prog_match(p) and p[0] and p[
                        6]: # check if program time matches current time, is active, and has a duration
                        duration = p[6] * gv.sd[
                            'wl'] / 100 * extra_adjustment # program duration scaled by "water level"
                        for b in range(len(p[7:7 + gv.sd[
                            'nbrd']])): # check each station for boards listed in program up to number of boards in Options
                            for s in range(8):
                                ...
                                if p[7 + b] & 1 << s: # if this station is scheduled in this program
                                    if gv.sd['seq']: # sequential mode
                                        ...
                                        if duration < gv.rs[sid][
                                            2]: # If duration is shortter than any already set for this station
                                            continue
                                        else:
                                            gv.rs[sid][2] = duration
...
class OSPy_app(web.application):
